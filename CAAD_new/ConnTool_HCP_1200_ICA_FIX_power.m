...
fid = fopen('/net/parasite/HCP/Scripts/slab/MDF/Danai/otherage_subjects.txt','r');
fid = fopen('/net/parasite/HCP/Scripts/slab/MDF/Power/ptn_included.txt','r');
fid = fopen('/net/parasite/HCP/Scripts/slab/Utilities/todo_minus_probs.txt','r');
fid = fopen('/net/parasite/HCP/Scripts/slab/Utilities/run3.txt','r');
fid = fopen('/net/parasite/HCP/Scripts/slab/MDF/Power/Retest.txt','r');
%fid = fopen('/net/parasite/HCP/Scripts/slab/Utilities/probs.txt','r');
s = textscan(fid,'%s');
s = s{1};
SubjDir = [s (mat2cell(ones(size(s,1),1),ones(size(s,1),1),1))];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (exist('SubjectDirList','var'))
    SubjectDirBatch = SubjectDirList;
else
    SubjectDirBatch = [];
end

if (exist('Batch','var'))
    SubjectDirBatch = ((Batch-1)*44+1):(Batch*44);
end

%SubjectDirBatch = SubjList;
fprintf(1,'%d\n',size(SubjDir,1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ImageTemplate = '[Exp]/derivatives/data/[Subject]/MNINonLinear/Results/slab/';
...
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
confoundFile = 'r3_rfMRI_REST';
confoundFile = ['r3_' RunDir{1}];
...
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
connectFile  = 's6_r3_rfMRI_REST';
connectFile = ['s6_r3_' RunDir{1}];
...
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
AnatomyMaskPath = '[Exp]/derivatives/data/[Subject]/MNINonLinear/slab/';

GreyFile  = '';
WhiteFile = 'r3_wm_ero.nii';
CSFFile   = 'r3_csf.nii';
...
EPIBrainMaskTemplate = ...
    '/net/parasite/HCP/ROIs/brain_mask_3mm.nii';
...
RealignmentParametersTemplate = ...
    '[Exp]/Subjects/[Subject]/MNINonLinear/Results/[Run]/Movement_Regressors.txt';
...
%%%  1 = include
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MotionDeriv = 0; %not included because derivatives are already in HCP file
MotionQuad = 1;
...
CensorParametersTemplate = ...
    '[Exp]/derivatives/MotionSummary/RestingState/CensorVector_[Subject]_[Run].mat';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ROICenters = [0 -48 26];
ROIfile = '/net/data2/OCD_NCA/Scripts/FirstLevel/DossenbachROIS_FromPower2011.csv';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
OutputTemplate  = '[Exp]/derivatives/[Subject]/ConnTool/[OutputName]/';
OutputName      = ['power_264_ICAFIX_p20f0b_' RunDir{1} '_CWcleanse'];
